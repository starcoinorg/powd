...
- `powd`
  ...
  - 在需要 runtime 工作时自举隐藏 daemon mode
- 隐藏的 `powd` daemon mode
...
公开 `powd` 入口负责用户意图和引导启动，但真正长期执行 miner 的还是隐藏 daemon mode。本地策略因此在 OpenClaw 关闭时仍然成立。
...
`powd mcp serve` 启动 stdio MCP server。

`powd mcp config` 输出标准本地 MCP 配置片段，固定包含：
...
- 如果 daemon 已运行，`powd` 通过私有 API 立即重配它
...
- 公开 `powd` 入口持有用户意图和持久化 profile
- 隐藏 daemon mode 持有运行时执行和自动预算控制
...
- 不会让公开入口持久化一套业务配置、daemon 启动参数又重复一套
