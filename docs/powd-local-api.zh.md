...
持久化用户配置只由 `powctl` 持有，不由 daemon 持有。配置内容为：
...
`powd` 通过 Unix socket 暴露 daemon 私有 JSON-RPC，供 `powctl`、dashboard 和诊断使用。
...
任何需要 daemon 的 `powctl` 命令都统一走：

1. `ctl` 读取持久化 profile
2. 如果 daemon 不在，则启动空白 `powd`
...
`powctl miner watch` 是面向人的 dashboard。
