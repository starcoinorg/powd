...
持久化用户配置只由公开 `powd` 入口持有，不由 daemon 持有。配置内容为：
...
- `mcp config` 总是输出绝对路径的 `powd` 和稳定的 `env: {}`

## MCP 工具面

`powd mcp serve` 暴露以下业务工具：
...
`powd` 通过 Unix socket 暴露 daemon 私有 JSON-RPC，供公开 `powd` 入口、dashboard 和诊断使用。
...
`powd` 以空白 daemon 形态启动，不接受 `--login`、`--pool` 之类公开业务参数。

任何需要 daemon 的 `powd` 命令都统一走：

1. `powd` 读取持久化 profile
2. 如果 daemon 不在，则启动 `powd` 的隐藏 daemon mode
3. `powd` 调用 `daemon.configure(profile)`
4. `powd` 再执行请求的业务动作
...
`powd miner watch` 是面向人的 dashboard。
